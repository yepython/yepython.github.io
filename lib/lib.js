...
runAsync: function(asyncFunc) {
	...
	showLauncherDialog: function() {
		// Розмітка вікна вже присутня статично в index.html (#launcher_dialog);
		// тут лише підставляються динамічні значення (тексти, що залежать від
		// selectedLang для кнопки копіювання/повідомлень, та базова адреса
		// середовища у підказці) і навішуються обробники подій.
		var $dlg = $('#launcher_dialog');
		...
		// Підставляємо реальну базову адресу середовища у плейсхолдер підказки
		// (текст самої підказки перекладається через data-i18n у translations.js).
		var $hint = $('#launcher_hint_text');
		$hint.html($hint.html().replace(/%LAUNCHER_BASE_URL%/g, PythonIDE.getLauncherBaseUrl()));

		// Скидаємо вікно до початкового стану (спосіб base64, приховані опції dpaste)
		$dlg.find('input[name="launcher_mode"][value="base64"]').prop('checked', true);
		$('#launcher_dpaste_options').hide();
		...
		$dlg.dialog({
			...
			close: function() { $dlg.dialog('destroy'); $dlg.hide(); }
		});

		// Вікно статичне й лишається у DOM між відкриттями, тому знімаємо
		// попередньо навішені обробники (.off), щоб уникнути дублювання.
		$('#launcher_url_text').off('click.launcher focus.launcher').on('click.launcher focus.launcher', function() { this.select(); });

		$dlg.find('input[name="launcher_mode"]').off('change.launcher').on('change.launcher', function() {
			var mode = $(this).val();
			$('#launcher_dpaste_options').toggle(mode === 'dpaste');
			refreshForMode(mode);
		});

		$('#launcher_dpaste_expiry').off('change.launcher').on('change.launcher', function() {
			// Термін дії змінено — перестворюємо сніпет із новим значенням.
			refreshForMode('dpaste');
		});

		var $copyBtn = $('#btn_copy_launcher');
		// .button() перетворює елемент на jQuery UI кнопку лише один раз —
		// вікно тепер статичне й може відкриватись повторно без перестворення DOM.
		if (!$copyBtn.hasClass('ui-button')) { $copyBtn.button(); }
		$copyBtn.off('click.launcher').on('click.launcher', function() {
