...
var PythonIDE = {
	...
	updateFileTabs: function() {
		// Ім'я поточного файлу над редактором (без вкладок)
		var displayCurrent = PythonIDE.currentFile;
		if(displayCurrent == 'mycode.py') {
			displayCurrent = PythonIDE.projectName + ".py";
		}
		$('#current_file_name').text(displayCurrent);
		// Перелік файлів у бічній панелі
		var html = '';
		for(var file in PythonIDE.files){
			var displayName = file;
			if(file == 'mycode.py'){
				displayName = PythonIDE.projectName + ".py";
			}
			var isSelected = (file == PythonIDE.currentFile);
			html += '<div class="sidebar_file_item' + (isSelected ? ' sidebar_file_selected' : '') + '" data-file="' + file + '">';
			html += '<span class="sidebar_file_name">' + displayName + '</span>';
			if(isSelected) {
				html += '<button type="button" class="btn_file_menu">⚙️</button>';
				html += '<div class="file_item_menu" style="display:none">';
				html += '<div class="file_item_menu_option" data-action="rename" data-i18n="menu_rename">' + (selectedLang === 'en' ? 'Rename' : 'Перейменувати') + '</div>';
				html += '<div class="file_item_menu_option" data-action="delete" data-i18n="menu_delete_file">' + (selectedLang === 'en' ? 'Delete file' : 'Видалити файл') + '</div>';
				html += '</div>';
			}
			html += '</div>';
		}
		// Файли-ресурси проєкту (наприклад, .png), що не редагуються у вкладках,
		// але входять до переліку файлів проєкту
		for(var assetName in PythonIDE.projectAssets) {
			html += '<div class="sidebar_file_item" data-file="' + assetName + '" data-asset="1">';
			html += '<span class="sidebar_file_name">🖼️ ' + assetName + '</span>';
			html += '<button type="button" class="btn_file_menu">⚙️</button>';
			html += '<div class="file_item_menu" style="display:none">';
			html += '<div class="file_item_menu_option" data-action="preview" data-asset="1" data-i18n="menu_preview">' + (selectedLang === 'en' ? 'Preview' : 'Переглянути') + '</div>';
			html += '<div class="file_item_menu_option" data-action="rename" data-asset="1" data-i18n="menu_rename">' + (selectedLang === 'en' ? 'Rename' : 'Перейменувати') + '</div>';
			html += '<div class="file_item_menu_option" data-action="delete" data-asset="1" data-i18n="menu_delete_file">' + (selectedLang === 'en' ? 'Delete file' : 'Видалити файл') + '</div>';
			html += '</div>';
			html += '</div>';
		}
		$('#file_sidebar_list').html(html);
		// Клік по назві файлу в переліку — відкрити його в редакторі
		$('#file_sidebar_list').off('click.sidebarfile').on('click.sidebarfile', '.sidebar_file_name', function(e) {
			e.stopPropagation();
			var $item = $(this).closest('.sidebar_file_item');
			var fileName = $item.attr('data-file');
			if($item.attr('data-asset') === '1') {
				PythonIDE.previewAsset(fileName);
			} else {
				PythonIDE.editFile(fileName);
			}
		});
		// Кнопка "↓" біля обраного файлу — показати/сховати спадне меню
		$('#file_sidebar_list').off('click.filemenu').on('click.filemenu', '.btn_file_menu', function(e) {
			e.stopPropagation();
			var $menu = $(this).siblings('.file_item_menu');
			$('.file_item_menu').not($menu).hide();
			$menu.toggle();
		});
		// Пункти спадного меню: перейменувати / видалити файл
		$('#file_sidebar_list').off('click.filemenuoption').on('click.filemenuoption', '.file_item_menu_option', function(e) {
			e.stopPropagation();
			var $item = $(this).closest('.sidebar_file_item');
			var fileName = $item.attr('data-file');
			var action = $(this).attr('data-action');
			var isAsset = $item.attr('data-asset') === '1';
			$(this).closest('.file_item_menu').hide();
			if(isAsset) {
				if(action === 'preview') {
					PythonIDE.previewAsset(fileName);
				} else if(action === 'rename') {
					PythonIDE.renamingAssetName = fileName;
					$('#txt_file_name').val(fileName);
					$('#file_settings').dialog('open');
				} else if(action === 'delete') {
					delete PythonIDE.projectAssets[fileName];
					PythonIDE.fsDeleteFile(fileName);
					PythonIDE.updateFileTabs();
				}
				return;
			}
			PythonIDE.renamingAssetName = null;
			if(action === 'rename') {
				...
				if(fileName === 'mycode.py') {
					PythonIDE.deleteMainFile();
				} else {
					delete PythonIDE.files[fileName];
					PythonIDE.fsDeleteFile(fileName);
					PythonIDE.editFile("mycode.py");
				}
			}
		});
		// Клік поза меню — закрити всі відкриті спадні меню
		$(document).off('click.closefilemenu').on('click.closefilemenu', function() {
			$('.file_item_menu').hide();
		});
	},
	// видалення файлу mycode.py: якщо в проєкті лишились інші файли,
	// один з них (пріоритетно .py) стає новим mycode.py; інакше створюється порожній
	deleteMainFile: function() {
		delete PythonIDE.files['mycode.py'];
		PythonIDE.fsDeleteFile('mycode.py');
		var remaining = [];
		for(var n in PythonIDE.files) { remaining.push(n); }
		if(remaining.length > 0) {
			var promote = null;
			for(var i = 0; i < remaining.length; i++) {
				if(/\.py$/i.test(remaining[i])) { promote = remaining[i]; break; }
			}
			if(!promote) promote = remaining[0];
			var content = PythonIDE.files[promote];
			delete PythonIDE.files[promote];
			PythonIDE.files['mycode.py'] = content;
			PythonIDE.fsRenameFile(promote, 'mycode.py', content);
			PythonIDE.projectName = promote.replace(/\.[^.]+$/, '');
		} else {
			PythonIDE.files['mycode.py'] = '';
			PythonIDE.projectName = 'mycode';
		}
	...
	// показати вміст файлу-ресурсу (наприклад, .png) у модальному вікні
	previewAsset: function(name) {
		var dataUrl = PythonIDE.projectAssets[name];
		if(!dataUrl) return;
		var $existing = $('#asset_preview_dialog');
		if($existing.length) { $existing.dialog('destroy'); $existing.remove(); }
		var $dlg = $('<div id="asset_preview_dialog"><img src="' + dataUrl + '" style="max-width:100%;height:auto;display:block;margin:0 auto;"></div>');
		$('body').append($dlg);
		$dlg.dialog({
			title: name,
			modal: true,
	...
	// допоміжна функція: гарантує, що window.fsToBrowse (внутрішня файлова
	// система) ініціалізовано
	ensureFsToBrowse: function() {
		try {
			if(typeof window.fsToBrowse === 'undefined' && typeof FileSystem !== 'undefined') {
				window.fsToBrowse = new FileSystem('epythonfs');
			}
		} catch(e) { console.warn('fsToBrowse init failed', e); }
		return window.fsToBrowse;
	},
	// видалити файл із внутрішньої файлової системи (якщо він там є)
	fsDeleteFile: function(name) {
		try {
			var fs = PythonIDE.ensureFsToBrowse();
			if(fs) fs.rm(name);
		} catch(e) { console.warn('fsToBrowse.rm failed', e); }
	},
	// перейменувати файл у внутрішній файловій системі: записати вміст
	// під новою назвою та видалити запис зі старою назвою
	fsRenameFile: function(oldName, newName, content) {
		try {
			var fs = PythonIDE.ensureFsToBrowse();
			if(fs) {
				fs.write(newName, content);
				fs.rm(oldName);
			}
		} catch(e) { console.warn('fsToBrowse rename failed', e); }
	},
	// "Додати до проєкту" — завантажити файл(и) з пристрою користувача
	// до внутрішньої файлової системи, додавши їх назви до переліку файлів проєкту
	addFileToProject: function(file) {
		var name = file.name.replace(/ \(\d+\)\./, ".");
		var ext = (name.split('.').pop() || '').toLowerCase();
		if(!/^(py|txt|json|csv|png)$/.test(ext)) {
			PythonIDE.showHint(selectedLang === 'en' ? 'Unsupported file format' : 'Непідтримуваний формат файлу');
			return;
		}
		...
		reader.onload = function(event) {
			...
			try {
				var fs = PythonIDE.ensureFsToBrowse();
				if(fs) fs.write(name, content);
			} catch(e) { console.warn('fsToBrowse.write failed', e); }
			if(ext === 'png') {
				delete PythonIDE.files[name];
				PythonIDE.projectAssets[name] = content;
			} else {
				delete PythonIDE.projectAssets[name];
				PythonIDE.files[name] = content;
			}
			PythonIDE.updateFileTabs();
			PythonIDE.showHint((selectedLang === 'en' ? 'File added: ' : 'Файл додано: ') + name);
		};
		reader.onerror = function() {
			PythonIDE.showHint(selectedLang === 'en' ? 'File read error' : 'Помилка читання файлу');
		};
		if(ext === 'png') reader.readAsDataURL(file);
		else reader.readAsText(file);
	},
	// "Зберегти проєкт" — запакувати всі файли проєкту в архів .yepp
	saveProjectZip: function() {
		if(typeof JSZip === 'undefined') {
			PythonIDE.showHint(selectedLang === 'en' ? 'Archiving library is not available' : 'Бібліотека архівації недоступна');
			return;
		}
		if(PythonIDE.files[PythonIDE.currentFile] !== undefined) {
			PythonIDE.files[PythonIDE.currentFile] = PythonIDE.editor.getValue();
		}
		var zip = new JSZip();
		var firstName = null;
		for(var f in PythonIDE.files) {
			if(firstName === null) firstName = f;
			var outName = (f === 'mycode.py') ? (PythonIDE.projectName + '.py') : f;
			zip.file(outName, PythonIDE.files[f]);
		}
		for(var a in PythonIDE.projectAssets) {
			if(firstName === null) firstName = a;
			var dataUrl = PythonIDE.projectAssets[a];
			var base64 = (dataUrl.split(',')[1]) || '';
			zip.file(a, base64, {base64: true});
		}
		var baseName = (firstName === null || firstName === 'mycode.py') ? PythonIDE.projectName : firstName.replace(/\.[^.]+$/, '');
		if(!baseName) baseName = PythonIDE.projectName || 'mycode';
		var zipName = baseName + '.yepp';
		zip.generateAsync({type: 'blob'}).then(function(blob) {
			saveAs(blob, zipName);
		}).catch(function(err) {
			console.error(err);
			PythonIDE.showHint(selectedLang === 'en' ? 'Failed to save project' : 'Не вдалося зберегти проєкт');
		});
	},
	// "Відкрити проєкт" — розпакувати архів .yepp: текстові файли стають
	// вкладками редактора, .png додаються до внутрішньої файлової системи,
	// усі файли проєкту потрапляють до переліку файлів проєкту
	openProjectZip: function(file) {
		if(typeof JSZip === 'undefined') {
			PythonIDE.showHint(selectedLang === 'en' ? 'Archiving library is not available' : 'Бібліотека архівації недоступна');
			return;
		}
		PythonIDE.saveSnapshot();
		var zipBaseName = file.name.replace(/\.yepp$/i, '');
		var reader = new FileReader();
		reader.onload = function(event) {
			JSZip.loadAsync(event.target.result).then(function(zip) {
				...
				zip.forEach(function(relPath, zipEntry) {
					...
					if(ext === 'png') {
						promises.push(zipEntry.async('base64').then(function(b64) {
							newAssets[name] = 'data:image/png;base64,' + b64;
						}));
					} else if(/^(py|txt|json|csv)$/.test(ext)) {
				...
				Promise.all(promises).then(function() {
					var mainKey = zipBaseName + '.py';
					if(newFiles[mainKey] !== undefined) {
						newFiles['mycode.py'] = newFiles[mainKey];
						if(mainKey !== 'mycode.py') delete newFiles[mainKey];
						PythonIDE.projectName = zipBaseName;
					} else {
						var pyName = null;
						for(var n in newFiles) { if(/\.py$/i.test(n)) { pyName = n; break; } }
						if(pyName) {
							newFiles['mycode.py'] = newFiles[pyName];
							if(pyName !== 'mycode.py') delete newFiles[pyName];
							PythonIDE.projectName = pyName.replace(/\.py$/i, '');
						} else {
							newFiles['mycode.py'] = '';
							PythonIDE.projectName = zipBaseName || 'mycode';
						}
					}
					try {
						if(typeof window.fsToBrowse === 'undefined' && typeof FileSystem !== 'undefined') {
						...
						if(window.fsToBrowse) {
							for(var an in newAssets) window.fsToBrowse.write(an, newAssets[an]);
						}
					} catch(e) { console.warn('fsToBrowse.write failed', e); }
					PythonIDE.files = newFiles;
					PythonIDE.projectAssets = newAssets;
					PythonIDE.currentFile = 'mycode.py';
					PythonIDE.disableChangeEvent = true;
					PythonIDE.editor.setValue(PythonIDE.files['mycode.py'] || '');
					PythonIDE.disableChangeEvent = false;
					PythonIDE.updateFileTabs();
					PythonIDE.editor.refresh();
					PythonIDE.showHint(selectedLang === 'en' ? 'Project loaded' : 'Проєкт завантажено');
				});
			}).catch(function(err) {
				console.error(err);
				PythonIDE.showHint(selectedLang === 'en' ? 'Failed to open project' : 'Не вдалося відкрити проєкт');
			});
		};
		reader.onerror = function() {
			PythonIDE.showHint(selectedLang === 'en' ? 'File read error' : 'Помилка читання файлу');
	...
	projectName: 'mycode',
	// назва файлу-ресурсу (.png), що зараз перейменовується через діалог
	// #file_settings; null, якщо діалог відкрито для звичайного файлу
	...
	files: {'mycode.py':''},
	// stores non-editable (binary) project files added via "Додати до проєкту"
	// or restored via "Відкрити проєкт" (e.g. .png), as {filename: dataURL}
	projectAssets: {},
		...
		repl: function(code) {
			...
			function step(r) {
				while(r.$isSuspension) {
					if(r.data.promise) {
						// FIX: previously the code only printed the raw promise
						// result here and never called r.resume(), so the
						// suspension was never actually completed - the while
						// loop just spun on the same (unchanged) r until the
						// 5s safety timeout fired. That's why import took ~5s
						// and afterwards module attributes were missing.
						r.data.promise.then(function(x) {
							// The resolved value (e.g. the fetched module's
							// source text from builtinRead) must be stored
							// in r.data.result before resuming - this is
							// where Skulpt's generated continuation code
							// looks for it. Without this, resuming just
							// continues with no module content, which is
							// why "import numpy" failed with ImportError
							// instead of actually loading the module.
							r.data.result = x;
							try {
								step(r.resume());
							} catch(e) {
							...
							// Mirror Sk.misceval.asyncToPromise's rejection
							// handling: stash the error and resume so the
							// interpreter can raise/handle it as a normal
							// Python exception, instead of just aborting.
							r.data.error = err;
							try {
								step(r.resume());
							} catch(e) {
						...
						return; // wait asynchronously; do not keep spinning
					} else {
						r = r.resume();
					}
...
runAsync: function(asyncFunc) {
	...
	runCode: function(runMode) {
		...
		// Закрити всі відкриті діалоги, окрім Консолі (#dlg)
		$(".ui-dialog-content").each(function() {
			if(this.id !== "dlg" && $(this).dialog("instance") && $(this).dialog("isOpen")) {
				$(this).dialog("close");
			}
		});
	...
	// Формує посилання-"ланчер", яке відкриває середовище ЄPython одразу із
	// вбудованим кодом програми (параметр ?code=, значення — base64 від
	// тексту програми). Адреса середовища береться з поточної сторінки, тому
	// ланчер лишається коректним, навіть якщо середовище розміщене за іншою
	// адресою.
	generateLauncherUrl: function() {
		// синхронізуємо останні зміни редактора з мапою файлів
		if (PythonIDE.currentFile) {
			PythonIDE.files[PythonIDE.currentFile] = PythonIDE.editor.getValue();
		}
		var code = (PythonIDE.files && PythonIDE.files['mycode.py'] !== undefined)
			? PythonIDE.files['mycode.py']
			: PythonIDE.editor.getValue();
		var encoded = '';
		try {
			// unescape(encodeURIComponent(...)) — коректно кодує юнікод перед btoa;
			// це точний зворотний бік декодування коду з параметра ?code= вище у файлі.
			encoded = window.btoa(unescape(encodeURIComponent(code || '')));
		} catch (e) {
			console.error('Launcher encode error:', e);
		}
		var baseUrl = PythonIDE.getLauncherBaseUrl();
		return baseUrl + '?code=' + encoded;
	},
	// Базова адреса середовища для ланчерів — береться з поточної сторінки
	// (window.location), а не хардкодиться, щоб посилання лишались коректними,
	// навіть якщо середовище перенесуть на іншу адресу.
	getLauncherBaseUrl: function() {
		return window.location.origin + window.location.pathname;
	},
	// Створює сніпет із поточним кодом програми на dpaste.com і повертає (через
	// callback) ланчер у формі yepython.pp.ua?paste=<код>. Сам файл не кодується
	// у посиланні — лише короткий ідентифікатор сніпета на dpaste.com.
	// expiryDays — термін дії сніпета в днях (1, 7, 30 або 365); за замовчуванням
	// dpaste.com використовує один тиждень.
	createDpasteLauncher: function(expiryDays, callback) {
		// синхронізуємо останні зміни редактора з мапою файлів
		if (PythonIDE.currentFile) {
			PythonIDE.files[PythonIDE.currentFile] = PythonIDE.editor.getValue();
		}
		...
		fetch('https://dpaste.com/api/v2/', {
			...
			if (!response.ok) { throw new Error('dpaste.com HTTP ' + response.status); }
			return response.text();
		}).then(function(pasteUrl) {
			pasteUrl = (pasteUrl || '').trim();
			// Відповідь API — повна адреса сніпета, напр. "https://dpaste.com/ABCD23456".
			// Із неї беремо лише короткий ідентифікатор для параметра ?paste=.
			var m = /([A-Za-z0-9]+)\/?$/.exec(pasteUrl);
			var pasteId = m ? m[1] : pasteUrl;
			if (!pasteId) { throw new Error('dpaste.com: empty response'); }
			callback(null, PythonIDE.getLauncherBaseUrl() + '?paste=' + encodeURIComponent(pasteId));
		}).catch(function(err) {
			console.error('dpaste.com create error:', err);
			callback(err);
		});
	},
	// Показує модальне вікно з готовим ланчером та кнопкою копіювання у буфер обміну.
	// Користувач обирає спосіб передачі коду:
	//  - base64   — код вбудовується прямо в посилання (?code=...);
	//  - dpaste.com — код зберігається як сніпет на dpaste.com, посилання коротке
	//                 (yepython.pp.ua?paste=<ідентифікатор>).
	showLauncherDialog: function() {
		var $existing = $('#launcher_dialog');
		if ($existing.length) { $existing.dialog('destroy'); $existing.remove(); }

		var titleText = selectedLang === 'en' ? 'Launcher for your program' : 'Запускач для вашої програми';
		var descText = selectedLang === 'en'
			? 'This link opens the environment and runs your program right away. Share it with anyone.'
			: 'Це посилання відкриває середовище програмування з вашою програмою. Ним можна поділитися з ким завгодно.';
		var modeLabel = selectedLang === 'en' ? 'Method:' : 'Спосіб:';
		var base64Label = selectedLang === 'en' ? 'Base64 (code is inside the link)' : 'Base64 (код вбудовано у посилання)';
		var dpasteLabel = selectedLang === 'en' ? 'dpaste.com (short link, needs internet)' : 'dpaste.com (коротке посилання, потрібен інтернет)';
		var expiryLabel = selectedLang === 'en' ? 'Expires in:' : 'Термін дії:';
		var expiryOptions = selectedLang === 'en'
			? [['1', 'a day'], ['7', 'a week'], ['30', 'a month'], ['365', 'a year']]
			: [['1', 'одна доба'], ['7', 'один тиждень'], ['30', 'один місяць'], ['365', 'один рік']];
		var copyText = selectedLang === 'en' ? 'Copy to clipboard' : 'Копіювати у буфер обміну';
		var copiedText = selectedLang === 'en' ? 'Copied!' : 'Скопійовано!';
		var loadingText = selectedLang === 'en' ? 'Creating a snippet on dpaste.com…' : 'Створення сніпета на dpaste.com…';
		var errorText = selectedLang === 'en'
			? 'Failed to create a snippet on dpaste.com. Check your internet connection and try again.'
			: 'Не вдалося створити сніпет на dpaste.com. Перевірте з’єднання з інтернетом і спробуйте ще раз.';
		var hintText = selectedLang === 'en'
			? 'Base64 works everywhere but makes a long link for big programs; dpaste.com gives a short link, but the snippet is deleted once its expiry time is up and opening the link needs internet access to dpaste.com. Tip: you can also host your code file anywhere on the web yourself and open it with a link like <b>' + PythonIDE.getLauncherBaseUrl() + '?load=&lt;URL to your file&gt;</b>.'
			: 'Base64 працює будь-де, але для великих програм посилання виходить довгим; dpaste.com дає коротке посилання, проте сніпет видаляється після завершення терміну дії, а для відкриття потрібен доступ до dpaste.com. Порада: код також можна розмістити самостійно за будь-якою адресою в інтернеті й відкрити його посиланням виду <b>' + PythonIDE.getLauncherBaseUrl() + '?load=&lt;адреса вашого файлу&gt;</b>.';

		var expiryOptionsHtml = '';
		for (var i = 0; i < expiryOptions.length; i++) {
			expiryOptionsHtml += '<option value="' + expiryOptions[i][0] + '"' + (expiryOptions[i][0] === '7' ? ' selected' : '') + '>' + expiryOptions[i][1] + '</option>';
		}

		var html = '<div id="launcher_dialog">' +
			'<p style="margin-top:0;">' + descText + '</p>' +
			'<div class="launcher_frame" style="border:1px solid var(--ide-border,#b9c8d9);border-radius:10px;padding:10px 14px;margin-bottom:12px;background:var(--ide-sidebar-bg,#f8f9fb);color:var(--ide-text,inherit);">' +
			'<div>' +
			'<b>' + modeLabel + '</b> ' +
			'<label style="margin-left:8px;cursor:pointer;"><input type="radio" name="launcher_mode" value="base64" checked> ' + base64Label + '</label> ' +
			'<label style="margin-left:12px;cursor:pointer;"><input type="radio" name="launcher_mode" value="dpaste"> ' + dpasteLabel + '</label>' +
			'</div>' +
			'<div id="launcher_dpaste_options" style="display:none;margin-top:8px;">' +
			'<label>' + expiryLabel + ' <select id="launcher_dpaste_expiry">' + expiryOptionsHtml + '</select></label>' +
			'</div>' +
			'</div>' +
			'<div class="launcher_frame" style="border:1px solid var(--ide-border,#b9c8d9);border-radius:10px;padding:10px 14px;margin-bottom:12px;background:var(--ide-sidebar-bg,#f8f9fb);color:var(--ide-text,inherit);font-size:0.83em;">' +
			'<i class="fa fa-info-circle"></i> ' + hintText +
			'</div>' +
			'<textarea id="launcher_url_text" readonly rows="5" style="width:100%;box-sizing:border-box;resize:none;font-family:monospace;font-size:0.82em;padding:6px;"></textarea>' +
			'<div style="margin-top:12px;text-align:right;">' +
			'<button type="button" id="btn_copy_launcher"><i class="fa fa-clipboard"></i> ' + copyText + '</button>' +
			'</div>' +
			'</div>';
		...
		// url зберігається у замиканні й оновлюється при зміні способу/перегенерації,
		// щоб кнопка копіювання завжди копіювала актуальне посилання.
		var url = PythonIDE.generateLauncherUrl();

		function setUrl(newUrl) {
			url = newUrl;
			$('#launcher_url_text').val(url);
		}
		setUrl(url);

		function refreshForMode(mode) {
			if (mode === 'dpaste') {
				var expiryDays = $('#launcher_dpaste_expiry').val();
				$('#launcher_url_text').prop('disabled', true).val(loadingText);
				PythonIDE.createDpasteLauncher(expiryDays, function(err, dpasteUrl) {
					$('#launcher_url_text').prop('disabled', false);
					if (err) {
						setUrl('');
						$('#launcher_url_text').val(errorText);
					} else {
						setUrl(dpasteUrl);
					}
				});
			} else {
				setUrl(PythonIDE.generateLauncherUrl());
			}
		}
		...
		$('#launcher_url_text').on('click focus', function() { this.select(); });

		$dlg.find('input[name="launcher_mode"]').on('change', function() {
			var mode = $(this).val();
			$('#launcher_dpaste_options').toggle(mode === 'dpaste');
			refreshForMode(mode);
		});

		$('#launcher_dpaste_expiry').on('change', function() {
			// Термін дії змінено — перестворюємо сніпет із новим значенням.
			refreshForMode('dpaste');
		});

		$('#btn_copy_launcher').button().on('click', function() {
			function onCopied() { PythonIDE.showHint(copiedText); }
			if (!url) { return; }
			if (navigator.clipboard && navigator.clipboard.writeText) {
				navigator.clipboard.writeText(url).then(onCopied).catch(function() {
					PythonIDE.legacyCopy(url);
					onCopied();
				});
			} else {
				PythonIDE.legacyCopy(url);
	...
	// Резервний спосіб копіювання у буфер обміну для браузерів без Clipboard API.
	...
	init: function(style) {
		// Ініціалізуємо fsToBrowse (jsfs) одразу при старті IDE, а не лениво
		// при першому відкритті діалогу файлового менеджера (filedialog.js).
		// Без цього Sk.builtin.file (customfile.js) отримує ReferenceError
		// при спробі fsToBrowse.read(...), і open() на існуючий файл кидає
		// хибний IOError "No such file or directory".
		if (typeof window.fsToBrowse === 'undefined' && typeof FileSystem !== 'undefined') {
			window.fsToBrowse = new FileSystem('epythonfs');
		}
		...
		// Подвійний клац по логотипу — створити "ланчер" (посилання, що одразу
		// запускає поточну програму у середовищі ЄPython)
		$('#logo').off('dblclick.launcher').on('dblclick.launcher', function(e) {
			e.stopPropagation();
			PythonIDE.showLauncherDialog();
		});
		// Подвійний клац по назві поточного файлу — вікно перейменування/видалення
		$('#current_file_name').off('dblclick.currentfile').on('dblclick.currentfile', function(e) {
			e.stopPropagation();
			var fileName = PythonIDE.currentFile;
			if(fileName === 'mycode.py') {
				$('#txt_project_name').val(PythonIDE.projectName);
		...
		// Кнопка "▸/◂" — показати/сховати бічну панель з переліком файлів
		$('#btn_toggle_file_sidebar').off('click.filesidebar').on('click.filesidebar', function(e) {
			e.stopPropagation();
			var $sidebar = $('#file_sidebar');
			if ($sidebar.is(':visible')) {
				$sidebar.hide();
				$('#holder').removeClass('sidebar_open');
				$(this).text('▸');
			} else {
				$sidebar.show();
				$('#holder').addClass('sidebar_open');
				$(this).text('◂');
			}
			// редактору потрібно перерахувати розміри/скрол після зміни ширини;
			// невелика затримка чекає завершення CSS-переходу
			setTimeout(function() {
				PythonIDE.editor.refresh();
			}, 210);
		});
		// Кнопка "✚" — створити новий файл
		$('#btn_new_file').off('click.newfile').on('click.newfile', function(e) {
			e.stopPropagation();
			var base = 'newfile', ext = '.txt', i = 2;
			var newFile = base + ext;
			while (PythonIDE.files[newFile] !== undefined) {
				newFile = base + i + ext;
				i++;
			}
		...
		// Кнопка "📤" — додати файл(и) до проєкту (у внутрішню файлову систему)
		$('#btn_add_file').off('click.addfileproject').on('click.addfileproject', function(e) {
			e.stopPropagation();
			document.getElementById('project_add_input').click();
		});
		document.getElementById('project_add_input').addEventListener('change', function() {
			var files = this.files;
			for(var i = 0; i < files.length; i++) {
				PythonIDE.addFileToProject(files[i]);
			}
			this.value = '';
		});
		// Кнопка "💾" — зберегти проєкт як архів .yepp
		$('#btn_save_project').off('click.saveproject').on('click.saveproject', function(e) {
			e.stopPropagation();
			PythonIDE.saveProjectZip();
		});
		// Кнопка "📂" — відкрити проєкт з архіву .yepp
		$('#btn_open_project').off('click.openproject').on('click.openproject', function(e) {
			e.stopPropagation();
			document.getElementById('project_open_input').click();
		});
		document.getElementById('project_open_input').addEventListener('change', function() {
			var file = this.files[0];
			if(file) PythonIDE.openProjectZip(file);
			this.value = '';
		});
		...
		$('#project_settings,#file_settings').dialog({
			...
			open: function() {
				var $dlg = $(this);
				setTimeout(function() {
					var $input = $dlg.find('input[type="text"]').first();
					$input.trigger('focus').select();
				}, 0);
			},
			close: function() {
				if(this.id === 'file_settings') PythonIDE.renamingAssetName = null;
			}
		...
		$('#file_settings').dialog('option', 'buttons', [
			{
				...
				click: function() {
					...
					if(PythonIDE.files[newFileName] || PythonIDE.projectAssets[newFileName] || (PythonIDE.projectName + ".py" == newFileName)) {
						PythonIDE.showHint(selectedLang === 'en' ? "A file with that name already exists" : "Файл із такою назвою вже існує");
						return;
					}
					if(PythonIDE.renamingAssetName) {
						// перейменування файлу-ресурсу (наприклад, .png)
						var oldAssetName = PythonIDE.renamingAssetName;
						if(!/\.png$/i.test(newFileName)) {
							PythonIDE.showHint(selectedLang === 'en' ? "File name must keep the .png extension" : "Назва файлу має зберігати розширення .png");
							return;
						}
						var assetContent = PythonIDE.projectAssets[oldAssetName];
						delete PythonIDE.projectAssets[oldAssetName];
						PythonIDE.projectAssets[newFileName] = assetContent;
						PythonIDE.fsRenameFile(oldAssetName, newFileName, assetContent);
						PythonIDE.renamingAssetName = null;
						PythonIDE.updateFileTabs();
						$(this).dialog('close');
						return;
					...
					var oldFileName = PythonIDE.currentFile;
					...
					PythonIDE.fsRenameFile(oldFileName, newFileName, fileContents);
					PythonIDE.updateFileTabs();
					$(this).dialog('close');
			...
			},
			{
				...
				click: function() {
					if(PythonIDE.renamingAssetName) {
						var assetToDelete = PythonIDE.renamingAssetName;
						delete PythonIDE.projectAssets[assetToDelete];
						PythonIDE.fsDeleteFile(assetToDelete);
						PythonIDE.renamingAssetName = null;
						PythonIDE.updateFileTabs();
						$(this).dialog('close');
						return;
					...
					delete PythonIDE.files[PythonIDE.currentFile];
					PythonIDE.fsDeleteFile(PythonIDE.currentFile);
					PythonIDE.editFile("mycode.py");
					$(this).dialog('close');
				}
			},
		...
		// Стрілки/пробіл під час виконання коду призначені для ігрових keyHandlers
		// (turtle/pygame тощо), але їх не можна блокувати, якщо фокус зараз у
		// текстовому полі (tkinter Text — <textarea>, Entry — <input>, звичайний
		// <input> тощо) — інакше користувач не може курсором переміщатись/виділяти
		// текст усередині такого віджета.
		function _isEditableTarget(e) {
			var tgt = e.target;
			if (!tgt) return false;
			var tag = tgt.tagName ? tgt.tagName.toLowerCase() : '';
			return tag === 'textarea' || tag === 'input' || tgt.isContentEditable === true;
		}
		$(window).keyup(function(e) {
			if(!PythonIDE.running) {
			    ...
			    if(!_isEditableTarget(e) && [37, 38, 39, 40].indexOf(e.keyCode) > -1) {
			        e.preventDefault();
			    }
		...
		$(window).keydown(function(e) {
			if(!PythonIDE.running) {
			    ...
			    if([37, 38, 39, 40].indexOf(e.keyCode) > -1) {
			        if (!_isEditableTarget(e)) e.preventDefault();
		...
		$('#radio_code_style').buttonset().change(function(e) {
			...
			switch(id) {
				case 'radio_code_style_light':
					...
					$('body').removeClass('theme-dark theme-dusk').addClass('theme-light');
					PythonIDE.setOption('code_style', 'light')
				break;
				case 'radio_code_style_dark':
					PythonIDE.editor.setOption("theme", "blackboard");
					$('body').css({'background-color': '#000'});
					$('body').removeClass('theme-light theme-dusk').addClass('theme-dark');
				...
				case 'radio_code_style_dusk':
					...
					$('body').removeClass('theme-light theme-dark').addClass('theme-dusk');
