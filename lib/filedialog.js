...
var FileManager = (function () {
  ...
  var _i18n = {
    uk: {
      ...
      notify_ext:        'Дозволені лише файли .txt, .csv та .png',
    ...
    en: {
      ...
      notify_ext:        'Only .txt, .csv and .png files are allowed',
  ...
  function render() {
    ...
    var html = '<div class="fm-root" style="font-family:var(--font-sans,system-ui,sans-serif);' +
      'color:var(--text-primary,#1a1a18);background:#e9edf2;padding:8px 8px 10px;' +
    ...
    html += '<div style="display:flex;flex-wrap:wrap;gap:6px;padding:8px 10px;' +
      'border-bottom:2px solid #999;margin-bottom:0">';
    ...
    html += '<input type="file" id="fm_fileInput" accept=".txt,.png,.csv" ' +
    ...
    /* Фрейм зі значками */
    html += '<div class="fm-list-frame" style="border:1px solid #bbb;border-top:none;' +
      'background:#fff;flex:1;min-height:100px;max-height:min(380px,55vh);overflow-y:auto;overscroll-behavior:contain;padding:4px 0">';
    ...
    var oldFrame = container.querySelector('.fm-list-frame');
    var savedScroll = oldFrame ? oldFrame.scrollTop : 0;

    container.innerHTML = html;

    var newFrame = container.querySelector('.fm-list-frame');
    if (newFrame) newFrame.scrollTop = savedScroll;

    //fitListFrame(container);
  }

  /* Вимірює реально доступний вертикальний простір у найближчому предку
     з обмеженою (не auto) висотою і виставляє списку файлів точний
     max-height, щоб решта вмісту (панель, хлібні крихти, рядок стану)
     завжди залишалась видимою, а прокрутка була лише в самому списку. */
  function fitListFrame(container) {
    var root = container.querySelector('.fm-root');
    var frame = container.querySelector('.fm-list-frame');
    if (!root || !frame) return;

    var bound = container.parentElement;
    while (bound && bound !== document.documentElement) {
      var cs = window.getComputedStyle(bound);
      if (cs.overflow === 'hidden' || cs.overflowY === 'hidden') break;
      bound = bound.parentElement;
    }
    if (!bound || bound === document.documentElement) return;

    frame.style.maxHeight = 'none';

    var boundHeight = bound.getBoundingClientRect().height;
    if (!boundHeight) { frame.style.maxHeight = 'min(380px,55vh)'; return; }

    var topGap = root.getBoundingClientRect().top - bound.getBoundingClientRect().top;
    var siblingsHeight = 0;
    Array.prototype.forEach.call(root.children, function (el) {
      if (el !== frame) siblingsHeight += el.offsetHeight;
    });

    var available = boundHeight - topGap - siblingsHeight - 12;
...
function openFileBrowser() {
  ...
  if (typeof $ !== 'undefined' && $('#filemanager').length) {
    ...
    /* центруємо при кожному відкритті — інакше вікно лишається там,
       де було востаннє (часто внизу поза видимою областю) */
    $dlg.dialog('option', 'position', { my: 'center', at: 'center', of: window });
    /* підлаштовуємо ширину, позицію та висоту списку при зміні розмірів вікна */
  ...
  box.style.cssText = 'background:var(--surface-2,#fff);border-radius:12px;' +
    ...
    'max-height:90vh;overflow:hidden;padding:0 0 12px;' +
