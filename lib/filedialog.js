...
var FileManager = (function () {
  ...
  var _i18n = {
    uk: {
      ...
      modal_move_title:  'Перемістити файл',
      modal_move_label:  'Введіть новий повний шлях призначення.',
      modal_move_btn:    'Перемістити',
      ...
      notify_sel_move:   'Виділіть файл або теку для переміщення.',
      notify_moved:      'Переміщено до «{dest}».',
      notify_move_err:   'Не вдалося перемістити файл.',
    ...
    en: {
      ...
      modal_move_title:  'Move file',
      modal_move_label:  'Enter the new full destination path.',
      modal_move_btn:    'Move',
      ...
      notify_sel_move:   'Select a file or folder to move.',
      notify_moved:      'Moved to "{dest}".',
      notify_move_err:   'Could not move the file.',
  ...
  function moveFile() {
    if (!_selected) { notify(t('notify_sel_move'), 'error'); return; }
    var srcFull = (_cwd === SEP ? SEP : _cwd) + (_cwd === SEP ? '' : '') + _selected;
    if (_cwd !== SEP) srcFull = _cwd + SEP + _selected;
    else srcFull = SEP + _selected;
    _moveTarget = { name: _selected, full: srcFull };
    showMoveModal(srcFull, function (destPath) {
      if (!destPath.trim()) return;
      var ok = _fs.mv(_moveTarget.full, destPath);
      if (ok) {
        _selected = null;
        _moveTarget = null;
        notify(t('notify_moved', {dest: destPath}), 'success');
      } else {
        notify(t('notify_move_err'), 'error');
      }
    });
  }
  ...
  var BT = 'display:inline-flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;padding:6px;width:52px;height:52px;font-size:11px;font-weight:400;background:#f4f4f4;border:1px solid #ccc;border-radius:50%;cursor:pointer;color:#333;font-family:inherit;';
  ...
  function showMoveModal(defaultValue, onConfirm) {
    _modalCb = onConfirm;
    var html = '<div id="fm_overlay" onclick="if(event.target.id===\'fm_overlay\')FileManager._closeModal()" ' +
      'style="position:fixed;inset:0;background:rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;z-index:9999">' +
      '<div style="background:var(--surface-2,#fff);border-radius:12px;border:0.5px solid var(--border,#ddd);padding:20px 24px;min-width:300px;max-width:440px;width:90%">' +
      '<div style="font-weight:500;margin-bottom:12px;font-size:15px">' + t('modal_move_title') + '</div>' +
      '<p style="font-size:13px;color:var(--text-secondary,#666);margin:0 0 8px">' + t('modal_move_label') + '</p>' +
      '<input id="fm_moveInput" type="text" value="' + escAttr(defaultValue) + '" ' +
        'onkeydown="if(event.key===\'Enter\')FileManager._confirmMove();if(event.key===\'Escape\')FileManager._closeModal()" ' +
        'style="width:100%;box-sizing:border-box;margin-bottom:14px;padding:7px 10px;font-size:13px;border:0.5px solid var(--border-strong,#bbb);border-radius:var(--radius,8px);font-family:inherit;color:var(--text-primary,#1a1a18);background:var(--surface-2,#fff)">' +
      '<div style="display:flex;gap:8px;justify-content:flex-end">' +
      '<button onclick="FileManager._closeModal()" style="' + BT_BASE + '">' + t('cancel') + '</button>' +
      '<button onclick="FileManager._confirmMove()" style="' + BT_ACCENT + '">' + t('modal_move_btn') + '</button>' +
      '</div></div></div>';
    _appendOverlay(html);
    setTimeout(function () {
      var inp = document.getElementById('fm_moveInput');
  ...
  function render() {
    ...
    html += '<div style="display:flex;flex-wrap:wrap;gap:6px;padding:8px 10px;' +
      'background:#f0f0f0;border:1px solid #bbb;border-bottom:2px solid #999;margin-bottom:0">';
    ...
    html += btn(t('move_file'),'ti-transfer',     'FileManager._moveFile()');
    ...
    if (_viewState) {
      html += '<div id="fm_viewOverlay" onclick="if(event.target.id===\'fm_viewOverlay\')FileManager._closeView()" ' +
        ...
        '<span style="margin-right:6px">&#128065;</span>' + escHtml(_viewState.name) + '</div>' +
