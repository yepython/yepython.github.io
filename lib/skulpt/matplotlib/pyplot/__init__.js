...
*/
mod.show = new Sk.builtin.func(function() {
    // Знищити існуючий графік, якщо є
    let chartStatus = Chart.getChart("myChart");
    if (chartStatus != undefined) {
        chartStatus.destroy();
    }
    ...
    // Визначаємо, чи графік — це діаграма (pie) чи щось інше
    var hasPie = Charts.some(ds => ds.type === "pie");
    var hasBar = Charts.some(ds => ds.type === "bar" || ds.type === "horizontalBar");
    var hasScatter = Charts.some(ds => ds.type === "scatter");
    ...
    var chartData;
    if (hasPie) {
        // Для pie використовуємо labels як числа або індекси
        chartData = {
            labels: xdata[0] && xdata[0].length ? xdata[0] : ydata[0].map((_, i) => i),
        ...
        // Для line, scatter, bar — використовуємо числові (x, y)
        var datasets = Charts.map((ds, i) => {
            let points = [];
            if (ds.type === "scatter" && Array.isArray(ds.data) && ds.data.length > 0 && ds.data[0].x !== undefined) {
                // scatter вже має {x,y}
                points = ds.data;
            } else {
                // Збираємо з xdata[i] та ydata[i]
                const xs = xdata[i] || [];
                const ys = ydata[i] || [];
    ...
    if (!hasPie) {
        chartOptions.scales = {
            x: {
                type: 'linear', // ← ключове: числовий масштаб
                display: xAxis,
                title: {
                    display: xLabelView,
                    text: xLabel,
    ...
    console.log('Chart displayed with numeric X axis');
});
// ----------------------------------------------------------
