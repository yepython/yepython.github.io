...
mod.show = new Sk.builtin.func(function() {
    ...
    if (hasPie) {
        ...
        var isNumericX = firstXs.length > 0 && firstXs.every(function(v) {
    ...
    var chartOptions = {
        ...
        responsive: true,
        plugins: {
            legend: { display: (hasPie && pieLabels) || hasAnyLabel || forceLegend, // легенда для pie з labels, якщо є хоч один label у датасетів, або явно викликали plt.legend()
					  position: 'top' },
            title: {
                display: !!chart_title,
                text: chart_title,
