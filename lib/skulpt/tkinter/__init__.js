...
var $builtinmodule = function(name) {
	...
	// Each time this module is (re-)loaded, the code below used to create
	// a brand new, private "timeouts" array + cleanup() closure. Any
	// setTimeout() chain started by a *previous* run (e.g. an animation
	// loop from a program whose window was closed but that was never
	// stopped) lived in the OLD closure and was therefore unreachable
	// here - nothing could ever cancel it, so it kept firing forever,
	// which is what caused two animations to appear to run at once.
	// Tracking pending timeouts on `window` instead makes them visible
	// (and cancellable) across module reloads, so we can clear out
	// anything left over from the previous run right here.
	...
	// Same idea as above, but for the real document-level keydown
	// listeners registered by commonKeyHandler() below. Without this,
	// a previous run's <Key> handler would keep reacting to keystrokes
	// forever (and pile up further with every new run), since nothing
	...
	s.Widget = new Sk.misceval.buildClass(s, function($gbl, $loc) {
		...
		function updateEventHandlers(self) {
			if (self.eventHandlers) {
				...
				function commonKeyHandler(ev) {
					...
					// Register a real, native keydown listener directly on
					// document instead of only pushing into the external
					// PythonIDE.keyHandlers array. Two reasons:
					//
					// 1) Using the CAPTURE phase (the "true" third arg)
					//    means this fires on the way DOWN to whatever
					//    element currently has focus - e.g. the code
					//    editor - before that element's own handlers get a
					//    chance to stopPropagation() on arrow keys (which
					//    editors commonly use for cursor movement). This
					//    makes <Key> work no matter what has DOM focus,
					//    without needing the canvas/window to be
					//    focusable at all.
					// 2) It no longer depends on however/whenever the
					//    surrounding IDE's own PythonIDE.keyHandlers array
					//    gets drained, which is what previously made keys
					//    work in a plain "while + sleep()" loop but
					//    silently stop working inside a
					//    doAnimate()/after() loop.
				...
				// <<ComboboxSelected>> — віртуальна подія ttk.Combobox.
				// Раніше не оброблялась узагалі: bind() лише клав callback
				// у self.eventHandlers, а тут не було відповідної гілки,
				// тож жоден DOM-обробник не прив'язувався і Python-функція
				// ніколи не викликалась при виборі елемента у списку.
		...
		var after = function(kwa, self, delay, callback) {
			...
			var timeoutId = setTimeout(function() {
				// The frame ran, so it's no longer "pending" - drop it from
				// the tracking list to avoid it growing forever across a
			...
			// Real tkinter's after() returns an id that can be passed to
			// after_cancel() - without this, Python code has nothing to
			// give after_cancel() and can never cancel a specific pending
			// frame (it previously got None back here).
		...
		$loc.after_cancel = new Sk.builtin.func(function(self, id) {
			// after_cancel() didn't exist at all before - any call to it
			// raised AttributeError, which meant a cancelled animation's
			// still-pending browser timer kept firing regardless.
...
//--------------        
		...
		var ensurePackStyles = function() {
			...
			$('head').append(
				...
				// tk_charsized-віджети (Button, Label, Entry, ...) в реальному
				// tkinter не переносять текст на новий рядок сам по собі —
				// перенос буває лише коли явно задано wraplength=. Раніше
				// 'white-space: nowrap' ставився лише коли задано width=
				// (applyWidgetStyles), тож Button/Label БЕЗ width= лишались
				// зі стандартним white-space:normal. pack() вимірює
				// "природну" ширину елемента (reqW) і потім прописує це
				// число як ЖОРСТКУ пікселову ширину в фінальному
				// position:absolute — і будь-яке найменше розходження між
				// виміряною і фактично відрендереною шириною (округлення,
				// шрифт, рендер кнопки в різних браузерах) призводило до
				// того, що текст впирався в межу і переносився вниз,
				// "вилазячи" за межі кнопки/лейбла. Ставимо nowrap за
				// замовчуванням для класу — wraplength= (нижче, в
				// applyWidgetStyles) все одно перекриває це інлайн-стилем
				// 'pre-line', бо inline style має вищий пріоритет за CSS-клас.
				// :not(textarea) — Text-віджет (<textarea class="tk_charsized">)
				// навпаки ПОВИНЕН переносити довгі рядки тексту, який
				// вводить користувач, тож його з цього правила виключаємо.
		...
		var grid = function(kwa, self) {
			...
			// ПРИМІТКА: зовнішні відступи (padx/pady) уже виставлені через
			// margin всередині commonDisplay() (вона читає ті самі props.padx/
			// props.pady). Тут НЕ можна повторно застосовувати їх ще й як
			// padding — інакше відступ між віджетами подвоюється (margin +
			// padding), а оскільки margin у grid-елементів CSS Grid не
			// схлопується, підсумковий проміжок між сусідніми рядками/
			// колонками виходить у 2 рази більшим за очікуваний.
			var el = $('#' + elementId);
			el.css({
	...
	s.Canvas = new Sk.misceval.buildClass(s, function($gbl, $loc) {
		...
		// Обчислює зсув (dx, dy) від точки прив'язки (anchor) до верхнього
		// лівого кута зображення заданого розміру. Винесено на рівень класу
		// Canvas (а не всередину create_image), щоб bbox() теж міг ним
		// користуватись для обчислення реальної бокс-рамки PhotoImage.
		...
		$loc.bbox = new Sk.builtin.func(function(self, item) {
			// ВАЖЛИВО: раніше bbox[2]/bbox[3] лишались "сирими" JS-числами
			// 0 (а не Sk.builtin.int_) для будь-якого елемента, чиї coords
			// не мають властивостей x2/y2 напряму (зображення, текст), а
			// для ліній/полігонів (де coords — це взагалі плаский масив
			// [x1,y1,x2,y2,...], а не {x1,y1,x2,y2}) ламались навіть
			// bbox[0]/bbox[1]. Кортеж з елементами різних типів (частина —
			// Sk.builtin.int_, частина — сирі JS-числа) і спричиняв аварію
			// при розпаковці на кшталт `x1, y1, x2, y2 = canvas.bbox(obj)`.
			//
			// Тепер bbox() коректно обчислює рамку для кожного типу
			// елемента і завжди повертає або кортеж з 4 Sk.builtin.int_,
			// або None (як справжній tkinter), якщо елемент не існує.
			if (!item) {
			...
			if (Array.isArray(e.coords)) {
				// line / polygon: плаский список [x1, y1, x2, y2, ...]
				var xs = [], ys = [];
				...
				// PhotoImage: сам елемент зберігає лише точку прив'язки
				// (coords.x1/y1), тому реальний розмір беремо із самого
				// PhotoImage (props.image) та враховуємо anchor.
				...
				// Текстовий елемент також зберігає лише точку прив'язки.
				// Приблизно оцінюємо розмір через canvas.measureText, щоб
				// повернути змістовну (а не вироджену) рамку.
				...
				// rectangle / oval / arc: coords вже містить повну рамку
				x1 = e.coords.x1;
				y1 = e.coords.y1;
				x2 = e.coords.x2;
				y2 = e.coords.y2;
			} else if (e.coords && e.coords.x1 !== undefined && e.coords.y1 !== undefined) {
				// запасний варіант для будь-якого іншого точкового елемента
				x1 = e.coords.x1;
				y1 = e.coords.y1;
				x2 = e.coords.x1;
				y2 = e.coords.y1;
			...
			// tkinter завжди нормалізує bbox так, щоб x1<=x2 та y1<=y2
...
s.Message = new Sk.misceval.buildClass(s, function($gbl, $loc) {
	var getHtml = function(self) {
		...
		// ВАЖЛИВО: у справжньому tkinter.Message розмір (width) задається в
		// ПІКСЕЛЯХ (screen units), а не в символах шрифту.
		if (!self.props.width) {
		...
		// white-space: pre-wrap зберігає явні \n з Python-рядка і водночас
		// автоматично переносить довгі рядки, щоб вони вписувались у заданий width.
		// word-wrap / overflow-wrap гарантують перенос дуже довгих слів без пробілів.
	...
	var init = function(kwa, self, master) {
		self.update = function() {
			...
			// Оновлюємо ширину та правила переносу тексту при зміні textvariable
...
t.Treeview = new Sk.misceval.buildClass(t, function($gbl, $loc) {
	...
	// 🟢 ВНУТРІШНІ JS-ФУНКЦІЇ (безпечні, не залежать від Sk.builtin.func)
	...
	// 🟢 ФІКС: раніше self.columns заповнювався лише один раз у __init__,
	// з kwarg columns=(...), переданого прямо в конструктор. Але типовий
	// tkinter-код найчастіше налаштовує колонки ПІСЛЯ створення віджета:
	//   tree = ttk.Treeview(root)
	//   tree["columns"] = ("name", "age")   # або tree.config(columns=...)
	// Базові __setitem__/configure/config (успадковані від Widget) лише
	// кладуть значення у self.props і НІКОЛИ не чіпають self.columns та
	// не перемальовують таблицю — тому self.columns лишався [] назавжди,
	// заголовки і <td> зі values() не рендерилися, і таблиця виглядала
	// порожньою, хоч insert() і відпрацьовував. Синхронізуємо self.columns
	// з self.props.columns і форсуємо renderHead_js/renderBody_js кожного
	// разу, коли щось змінюється через __setitem__/config/configure.
	...
	// Експортуємо render-методи для Python (якщо знадобляться)
	...
	// --- Публічні методи (З АВТОМАТИЧНОЮ ІНІЦІАЛІЗАЦІЄЮ) ---
	
	var heading_func = function(kwa, self, column) {
		if (!self.headings) self.headings = {}; // 🟢 Захист від undefined
	...
	var column_func = function(kwa, self, column) {
		if (!self.colOptions) self.colOptions = {}; // 🟢 Захист від undefined
		var col = Sk.ffi.remapToJs(column);
		var props = unpackKWA(kwa);
	...
	var item_func = function(kwa, self, item) {
		if (!self.items) self.items = {};
		...
		if (Object.keys(props).length === 0) {
			var res = {
				...
				"values": Sk.ffi.remapToPy(data.values || []),
...
var filedialog = function(name) {
	...
	// Helper to read file from browser and save to Virtual FS
	function processFile(file, callback) {
		...
		reader.onload = function(e) {
			var vPath = "/" + file.name;
			try {
				Sk.__jsfs.write(vPath, e.target.result);
			} catch(ex) {
				console.warn("Could not write to VFS:", ex);
			}
			callback(vPath);
		};
	...
	// 🟢 НОВА ФУНКЦІЯ: Правильний парсинг filetypes для HTML accept
			...
			if (Array.isArray(types)) {
				types.forEach(function(t) {
					if (Array.isArray(t) && t.length > 1) {
						// Розбиваємо по пробілах (наприклад, "*.png *.jpg")
						var exts = t[1].split(/\s+/);
						exts.forEach(function(ext) {
							ext = ext.trim();
							if (!ext) return;
							if (ext === '*' || ext === '*.*') {
								accept.push('*/*'); // Приймаємо всі файли
							} else if (ext.startsWith('*.')) {
								accept.push(ext.substring(1)); // "*.png" -> ".png"
							} else if (ext.startsWith('.')) {
								accept.push(ext); // ".png" -> ".png"
							}
						});
					}
				});
	...
	var askopenfilename = function(kwa) {
		...
		return Sk.misceval.promiseToSuspension(new Promise((resolve) => {
			...
			var acceptStr = parseFiletypes(props.filetypes);
			if (acceptStr) input.accept = acceptStr;
			
			document.body.appendChild(input);
			input.addEventListener('change', function() {
				if (input.files.length > 0) {
					processFile(input.files[0], function(path) {
						resolve(new Sk.builtin.str(path));
						input.remove();
					});
				} else {
	...
	var askopenfilenames = function(kwa) {
		...
		return Sk.misceval.promiseToSuspension(new Promise((resolve) => {
			...
			input.addEventListener('change', function() {
				...
				if (files.length === 0) {
					resolve(new Sk.builtin.tuple([]));
					input.remove();
					return;
				}
				
				files.forEach(function(file) {
					processFile(file, function(path) {
						paths.push(new Sk.builtin.str(path));
						processed++;
						if (processed === files.length) {
							resolve(new Sk.builtin.tuple(paths));
							input.remove();
						}
					});
				});
	...
	var asksaveasfilename = function(kwa) {
		...
		var title = props.title ? Sk.ffi.remapToJs(props.title) : "Save As";

		return Sk.misceval.promiseToSuspension(new Promise((resolve) => {
			var dialogId = "filedialog_save_" + Math.random().toString(36).substring(2);
			var html = `
				<div id="${dialogId}" title="${title}" style="overflow-x: hidden;">
					<p>File name:</p>
					<input type="text" id="${dialogId}_input" value="${initialFile}" style="width:100%; margin-top:5px;" autofocus>
				</div>
			`;
			$("body").append(html);
			var $dialog = $("#" + dialogId);
			var $input = $("#" + dialogId + "_input");

			function finishSave() {
				var val = $input.val();
				$dialog.dialog("close");
				$dialog.remove();
				resolve(new Sk.builtin.str(val));
			}

			$input.on("keypress", function(event) {
				if (event.which === 13) finishSave();
			});
	...
	var askdirectory = function(kwa) {
		...
		var title = props.title ? Sk.ffi.remapToJs(props.title) : "Select Directory";

		return Sk.misceval.promiseToSuspension(new Promise((resolve) => {
			var dialogId = "filedialog_dir_" + Math.random().toString(36).substring(2);
			var html = `
				<div id="${dialogId}" title="${title}" style="overflow-x: hidden;">
					<p>Directory path:</p>
					<input type="text" id="${dialogId}_input" value="${initialDir}" style="width:100%; margin-top:5px;" autofocus>
				</div>
			`;
			$("body").append(html);
			var $dialog = $("#" + dialogId);
			...
			$input.on("keypress", function(event) {
				if (event.which === 13) finishSelect();
			});
