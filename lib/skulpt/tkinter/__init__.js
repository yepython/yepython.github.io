...
var $builtinmodule = function(name) {
	...
	var applyWidgetStyles = function(self) {
		...
		if (self.props.font) {
			...
			// Reserve enough box height for the font itself when no explicit height= was
			// given (e.g. a big display font on a Label). Without this, a large font-size
			// can be applied after the surrounding layout (CSS Grid row / pack box) already
			// measured a smaller natural height, so the glyph visually pokes out of its box —
			// real Tk never has this issue since it always sizes the widget to fit the font.
			if (e.hasClass("tk_charsized") && !self.props.height) {
				e.css('min-height', '1.3em');
			}
		}
					...
					function handleKeydownEvent(e) {
						...
						switch (e.key) {
							...
							case "ArrowRight":
								...
								break;
							case " ":
								// Browsers report the space bar's e.key as a literal " " character;
								// real Tk reports the keysym as the word "space" (event.char is still " ").
								event.keysym = "space";
								break;
							default:
								event.keysym = e.key;
...
// skipGeometryStyling=true (new pack()) skips fill/expand/margin so layoutPack() fully controls geometry; grid()/place() still use the old path
		...
		var grid = function(kwa, self) {
			...
			// Read grid params
			// Real Tk options are Tcl strings, so tkinter happily accepts row=5 or row='5'.
			// Coerce to Number here too, otherwise a string value (e.g. column='1') makes
			// the '+1' below do string concatenation ('1'+1 === '11') instead of arithmetic,
			// and the widget gets placed far outside the intended column.
			var row = Number(Sk.ffi.remapToJs(props.row ?? 0)) || 0;
			var column = Number(Sk.ffi.remapToJs(props.column ?? 0)) || 0;
			var rowspan = Number(Sk.ffi.remapToJs(props.rowspan ?? 1)) || 1;
			var columnspan = Number(Sk.ffi.remapToJs(props.columnspan ?? 1)) || 1;
	...
	s.Canvas = new Sk.misceval.buildClass(s, function($gbl, $loc) {
		...
		$loc.move = new Sk.builtin.func(function(self, item, dx, dy) {
			var id = Sk.ffi.remapToJs(item);
			var jdx = Sk.ffi.remapToJs(dx);
			var jdy = Sk.ffi.remapToJs(dy);

			if (self && self.elements && self.elements[id] && !self.elements[id].deleted) {
				var c = self.elements[id].coords;
				if (Array.isArray(c)) {
					// line/polygon: flat list [x1, y1, x2, y2, ...] — shift every point, not a single x1/y1/x2/y2 box
		...
		var create_polygon = function(kwa, self, coords) {
			...
			return commonCanvasElement(self, {
				...
				draw: function(canvas) {
					...
					if (props.smooth) {
						smooth = Sk.ffi.remapToJs(props.smooth)
						if (smooth) {
					...
					if (props.fill && Sk.ffi.remapToJs(props.fill) != '') {
						cx.fillStyle = Sk.ffi.remapToJs(props.fill);
						cx.fill();
					}
				}
			});
...
$loc.create_text = new Sk.builtin.func(create_text);
	...
	$loc.create_image = new Sk.builtin.func(create_image);
		...
		// The canvas element is created with tabindex="0" (see getHtml above), so it's a real
		// focusable DOM element. Moving browser focus onto it here matters beyond Tk semantics:
		// it also takes focus off whatever HTML button the page's IDE last focused (e.g. a
		// Run/Stop button), which otherwise intercepts keys like Space/Enter as a button click.
		...
		var init = function(kwa, self, master) {
			...
			self.update = function() {
				...
				if (self.props.image) {
					...
					// Recompute the auto width every update (not just the one time it happens to
					// equal 1) so the field keeps growing as text gets longer. The previous check
					// `self.props.width === 1` only ever fired once, on the 1-char -> 2-char
					// transition, then silently stopped resizing for anything longer.
					if (self._autoWidth) {
						self.props.width = v.length;
					}
					if (self.props.width) {
						// min-width, not width: a fixed width overrides the grid's
						// justify-self:stretch, making the box shrink to content and
						// appear to jump to the left instead of filling its cell.
						$('#tkinter_' + self.id).css({ 'min-width': Sk.ffi.remapToJs(self.props.width) + 'ch', 'width': '' });
					}
				}
