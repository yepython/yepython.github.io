...
var pack = function(kwa, self) {
    ...
    let parentHeight = Sk.ffi.remapToJs(self.master.props.height) - 55;

    // Додавання CSS один раз
    if (!$('#tkinter-pack-style').length) {
        $('head').append(`
            <style id="tkinter-pack-style">
                .flex-root { display: flex; flex-direction: column; width: 100%; }
                .flex-section { display: flex; width: 100%; box-sizing: border-box; align-items: center; }
                .flex-main { display: flex; flex: 1; flex-direction: row; width: 100%; justify-content: space-between; }
                /* left_side/center/right_side — це діти .flex-main (рядок), а не
                   .flex-root (колонка), як up_side/down_side. Клас .flex-section
                   задає width:100%, що коректно для колонкового контексту
                   (up_side/down_side), але всередині рядка flex-main це змушує
                   КОЖНУ з трьох секцій (left/center/right) одночасно вимагати
                   100% ширини — вони починають перекриватись і "заходити" одна
                   на одну. Тут скидаємо ширину до auto: секція завжди займає
                   РІВНО стільки місця, скільки потребує її вміст (flex: 0 0 auto
                   — без grow і БЕЗ shrink), і ніколи не стискається менше за
                   нього. Якщо стискати дозволити (flex-shrink:1 + min-width:0,
                   як було раніше), а віджет усередині (наприклад Label із
                   картинкою) сам не стискається — контейнер ставав вужчим за
                   свій вміст, і картинка вилазила за його межі. Тепер розмір
                   вікна не чіпаємо — підлаштовується лише сам контейнер. */
                .flex-main > .flex-section, .flex-main > .flex-center {
                    width: auto;
                    flex: 0 0 auto;
                    align-self: stretch;
                }
    ...
    // Створюємо flex-root одразу з усією структурою
    if (!parentJQ.find('.flex-root').length) {
        parentJQ.append(`
            <div id="flex-root" class="flex-root">
                <div id="up_side" class="flex-section column"></div>
                <div class="flex-main">
                    <div id="left_side" class="flex-section row"></div>
                    <div id="center" class="flex-center"></div>
                    <div id="right_side" class="flex-section row-reverse"></div>
                </div>
                <div id="down_side" class="flex-section column"></div>
            </div>
        `);
    }
    ...
    // left_side / center / right_side тепер створюються одразу, у фіксованому
    // порядку (див. вище), а НЕ лениво при першому pack(side=...). Раніше
    // секції додавались у .flex-main у порядку виклику pack(): якщо, наприклад,
    // side="right" викликався раніше за side="left", right_side опинявся
    // ПЕРШОЮ дитиною flex-main (яка має звичайний flex-direction: row) —
    // тобто візуально ЗЛІВА, а left_side, доданий пізніше, — справа.
    // margin-left:auto на right_side рятував ситуацію лише частково і
    // ламався, щойно left_side йшов у DOM після нього. Фіксований порядок
    // прибирає залежність від порядку викликів pack() у коді користувача.
    ...
    flexRoot.css('height', parentHeight + "px");

    // Стилізація елемента
const el = $('#' + elementId);
let fillVal = props.fill ? Sk.ffi.remapToJs(props.fill) : 'none';
let expandVal = props.expand ? Sk.ffi.remapToJs(props.expand) : false;
// Скидаємо стилі
el.css({ margin: 0, padding: 0, width: 'auto', height: 'auto', flex: '' });

// fill
if (fillVal === 'x') {
    el.css({ width: '100%' });
} else if (fillVal === 'y') {
    el.css({ height: '100%', alignSelf: 'stretch' });
} else if (fillVal === 'both') {
    el.css({ width: '100%', height: '100%', alignSelf: 'stretch' });
}

// expand — дає елементу flex: 1, щоб він займав вільний простір
if (expandVal) {
    el.css({ flex: '1' });
    if (fillVal === 'x' || fillVal === 'both') el.css({ width: '100%' });
    if (fillVal === 'y' || fillVal === 'both') el.css({ alignSelf: 'stretch' });
}

    const placeJQ = flexRoot.find('#' + targetId);

// Зберігаємо pack-параметри в self
if (props.fill) self._pack_fill = Sk.ffi.remapToJs(props.fill);
if (props.expand) self._pack_expand = Sk.ffi.remapToJs(props.expand);
    // Вставка елемента
    commonDisplay(kwa, self, placeJQ);

    // Налаштування anchor
    // ВАЖЛИВО: commonDisplay() видаляє старий DOM-елемент і рендерить
    // новий з нуля (self.getHtml() + append), тому align-self треба
    // ставити ПІСЛЯ commonDisplay(), інакше стилі застосовуються до вже
    // видаленого елемента і губляться. up_side/down_side — це ОДИН
    // спільний flex-контейнер для ВСІХ віджетів, запакованих з цього
    // боку. align-items — властивість контейнера, а не дитини: якщо
    // виставити її тут, вона "протікає" на ВСІ віджети в цьому
    // контейнері, а не лише на поточний. Тому виставляємо align-self на
    // самому елементі: це per-child override, який не займає значення
    // за замовчуванням контейнера (align-items:center із CSS-класу
    // .flex-section) для решти дітей.
    const elAfter = $('#' + elementId);
    // Контейнери up_side/down_side мають flex-direction: column, тому їхня
    // поперечна вісь — ГОРИЗОНТАЛЬНА, і саме літери w/e (захід/схід) керують
    // align-self. Натомість left_side/right_side/center — це flex-direction:
    // row (чи row-reverse), тому поперечна вісь — ВЕРТИКАЛЬНА, і керувати
    // align-self мають літери n/s (північ/південь), а w/e тут ролі не
    // відіграють. Раніше n/s ніде не оброблялись — anchor="n" для
    // side="right"/"left" просто ігнорувався, і елемент лишався
    // вертикально по центру (align-items:center за замовчуванням), звідки
    // й "картинка нижче, ніж в оригіналі".
    const crossAxisIsHorizontal = (pos === 'top' || pos === 'bottom');
    if (props.anchor) {
        let anchor = Sk.ffi.remapToJs(props.anchor);
        if (anchor === "center") {
            elAfter.css('align-self', "center");
        } else if (crossAxisIsHorizontal) {
            if (anchor.includes("w")) {
                elAfter.css('align-self', "flex-start");
            } else if (anchor.includes("e")) {
                elAfter.css('align-self', "flex-end");
            }
        } else {
        ...
        elAfter.css('align-self', "");
    }

    // Притискаємо right_side до правого краю flex-main
    if (pos === 'right') {
        flexRoot.find('#right_side').css('margin-left', 'auto');
    }

	// Для bottom - переміщуємо елемент на початок контейнера
	if (pos === 'bottom') {
		const el = $('#' + elementId);
		placeJQ.prepend(el);
	}
...
// Frame ---
	s.Frame = new Sk.misceval.buildClass(s, function($gbl, $loc) {
		var getHtml = function(self) {
			...
			return '<div id="tkinter_' + self.id + '" class="tk_pixelsized" style="margin:auto;width:' + width + 'px; height:' + height + 'px; overflow:hidden;"></div>';
		}
