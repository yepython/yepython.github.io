...
// Зберігаємо в window, щоб діалог знаходився при повторних запусках модуля
// (кожен запуск Skulpt завантажує __init__.js заново, тому локальні var скидаються)
if (!window.__turtleDialogInstance)  { window.__turtleDialogInstance  = null; }
if (!window.__turtleCanvasContainer) { window.__turtleCanvasContainer = null; }
...
function updateDialogSize(width, height) {
    if (typeof $ !== "undefined" && $.fn && $.fn.dialog && window.__turtleDialogInstance) {
        var $dialog = window.__turtleDialogInstance.parent();
        ...
        window.__turtleDialogInstance.dialog("option", "width",  width);
        window.__turtleDialogInstance.dialog("option", "height", dialogHeight);
...
function createjQueryDialog(width, height) {
    ...
    if (window.__turtleDialogInstance) {
        try {
            window.__turtleDialogInstance.dialog("destroy");
        } catch (e) {}
        window.__turtleDialogInstance.remove();
        window.__turtleDialogInstance  = null;
        window.__turtleCanvasContainer = null;
    }

    // Create container div
    window.__turtleCanvasContainer = document.createElement("div");
    window.__turtleCanvasContainer.id = "turtle-dialog-" + Date.now();
    window.__turtleCanvasContainer.title = "Turtle Graphics";
    document.body.appendChild(window.__turtleCanvasContainer);

    // Create jQuery UI dialog
    window.__turtleDialogInstance = $(window.__turtleCanvasContainer).dialog({
        width: width,
        height: height,
        modal: false,
        resizable: false,
    ...
    window.__turtleCanvasContainer.appendChild(targetEl);
...
var targetEl = (function getConfiguredTarget() {
    ...
    if (typeof $ !== "undefined" && $.fn && $.fn.dialog) {
        // Create jQuery dialog automatically (createjQueryDialog destroys any existing dialog)
        ...
        // Знищуємо прив'язаний turtleInstance, щоб модуль ініціалізувався заново,
        // а не просто викликав reset() на старому екземплярі
        if (el.turtleInstance) {
            el.turtleInstance = null;
        }
...
targetEl.turtleInstance = (function generateTurtleModule(targetEl) {
    ...
    /*  Asset loader                                                     */
    /* ---------------------------------------------------------------- */
    // Розширення, які перебираємо автоматично якщо name не має розширення
    var IMAGE_EXTENSIONS = ["png", "gif", "jpg", "jpeg", "svg", "bmp", "webp"];

    // Повертає true, якщо рядок виглядає як ім'я з розширенням (має крапку не на початку)
    function hasImageExtension(name) {
        var dot = name.lastIndexOf(".");
        return dot > 0 && dot < name.length - 1;
    }

    // Спроба завантажити зображення з рядка (URL або data URL)
    function loadImgFromSrc(src, name, resolve, reject) {
        var img = new Image();
        img.onload  = function () { cfg.assets[name] = this; resolve(img); };
        img.onerror = function () { reject(new Error("Cannot load asset: " + src)); };
        img.src = src;
    }

    // Спроба прочитати один ключ з VFS; повертає Promise<HTMLImageElement> або null
    function tryLoadFromVFS(key, storeAs) {
        if (typeof window === "undefined" || !window.fsToBrowse) return null;
        try {
            var fileData = window.fsToBrowse.read(key);
            if (!fileData) return null;
            return new Promise(function (resolve, reject) {
                var img = new Image();
                img.onload  = function () { cfg.assets[storeAs] = this; resolve(img); };
    ...
    function getAsset(name) {
        ...
        // 1. assets повернув рядок-URL → завантажуємо як раніше
        if (typeof res === "string") {
            return new Promise(function (resolve, reject) {
                loadImgFromSrc(res, name, resolve, reject);
            });
        }

        // 2. assets повернув готовий Image/об'єкт → миттєво
        if (res) {
            return new InstantPromise(undefined, res);
        }

        // 3. Не знайдено в assets — шукаємо у VFS
        //    3a. Точний збіг по імені (наприклад "duck.png")
        var vfsExact = tryLoadFromVFS(name, name);
        if (vfsExact) return vfsExact;

        //    3b. Якщо name БЕЗ розширення — перебираємо суфікси
        if (!hasImageExtension(name)) {
            for (var i = 0; i < IMAGE_EXTENSIONS.length; i++) {
                var candidate = name + "." + IMAGE_EXTENSIONS[i];
                // Спочатку перевіряємо assets під повним ім'ям
                var candidateRes = (typeof assets === "function") ? assets(candidate) : assets[candidate];
                if (typeof candidateRes === "string") {
                    return new Promise(function (resolve, reject) {
                        loadImgFromSrc(candidateRes, name, resolve, reject);
                    });
                }
                if (candidateRes) return new InstantPromise(undefined, candidateRes);
                // Потім VFS
                var vfsCandidate = tryLoadFromVFS(candidate, name);
                if (vfsCandidate) return vfsCandidate;
            }
        }

        return new InstantPromise(undefined, undefined);
    }
        ...
        * ================================================================ */
        ...
        proto.$shape = function (name) {
            if (name) {
                ...
                if (!shapes[name] && !hasImageExtension(name)) {
                    // Спробуємо з розширеннями
                    for (var _i = 0; _i < IMAGE_EXTENSIONS.length; _i++) {
                        var _cand = name + "." + IMAGE_EXTENSIONS[_i];
                        if (shapes[_cand]) { resolvedName = _cand; break; }
                    }
                }
                if (shapes[resolvedName]) {
                    this._shape = resolvedName;
                    return this.addUpdate(undefined, this._shown, { shape: resolvedName });
                }
            }
            return this._shape;
        };
    ...
    (function (proto) {
        ...
        proto.$title = function (t) {
            if (typeof $ !== "undefined" && $.fn && $.fn.dialog && window.__turtleDialogInstance) {
                window.__turtleDialogInstance.dialog("option", "title", t);
            } else {
                document.title = t;
            }
        };
        ...
        proto.$bye = function () { 
            ...
            if (window.__turtleDialogInstance) {
                window.__turtleDialogInstance.dialog("close");
            }
        };
