 ...
 *  - Додано: pygame.image.tostring/fromstring/tobytes/frombytes/frombuffer/tobuffer
 *  - Додано: Surface.tobytes/tostring/tobuffer (pygame 2.x API)
 */
...
function surface_class($gbl, $loc) {
    ...
    // ─── Surface.tobytes / tostring / tobuffer / frombytes / fromstring ─────────
    // Спільна допоміжна: Surface → Skulpt bytes за форматом пікселів
    function _surf_to_bytes(self, format, flipped) {
        var fmt = (format !== undefined && format !== Sk.builtin.none.none$)
            ? Sk.ffi.remapToJs(format).toUpperCase() : 'RGBA';
        var flip = (flipped !== undefined && flipped !== Sk.builtin.none.none$)
            ? !!Sk.ffi.remapToJs(flipped) : false;
        var w = self.width, h = self.height;
        var raw = self.context2d.getImageData(0, 0, w, h).data;
        var ch;
        if      (fmt === 'RGBA')          ch = [0,1,2,3];
        else if (fmt === 'RGB' || fmt === 'P') ch = [0,1,2];
        else if (fmt === 'ARGB')          ch = [3,0,1,2];
        else if (fmt === 'BGRA')          ch = [2,1,0,3];
        else if (fmt === 'BGR')           ch = [2,1,0];
        else if (fmt === 'RGBX')          ch = [0,1,2,3];
        else throw new Sk.builtin.ValueError('Unsupported format: ' + fmt);
        var bpp = ch.length;
        var out = new Uint8Array(w * h * bpp);
        var idx = 0;
        for (var row = 0; row < h; row++) {
            var y = flip ? (h - 1 - row) : row;
            for (var x = 0; x < w; x++) {
                var src = (y * w + x) * 4;
                for (var c = 0; c < bpp; c++) {
                    out[idx++] = (fmt === 'RGBX' && c === 3) ? 255 : raw[src + ch[c]];
                }
            }
        }
    ...
    // surface.tobytes(format='RGBA', flipped=False) -> bytes
    $loc.tobytes  = new Sk.builtin.func(function(self, format, flipped) {
        return _surf_to_bytes(self, format, flipped);
    });
    // tostring — старе ім'я pygame 1.x, ідентично tobytes
    $loc.tostring = new Sk.builtin.func(function(self, format, flipped) {
        return _surf_to_bytes(self, format, flipped);
    });
    // tobuffer — синонім (pygame не має окремого Surface.tobuffer, але для симетрії)
    $loc.tobuffer = new Sk.builtin.func(function(self, format, flipped) {
        return _surf_to_bytes(self, format, flipped);
    });
...
function makeImageModule() {
    ...
    mod.save = new Sk.builtin.func(function (surf, filename) {
        ...
        // Use a fresh canvas at exact pixel size to avoid devicePixelRatio artifacts
        var w = surf.offscreen_canvas.width;
        var h = surf.offscreen_canvas.height;
        var exportCanvas = document.createElement('canvas');
        exportCanvas.width  = w;
        exportCanvas.height = h;
        var exportCtx = exportCanvas.getContext('2d');
        exportCtx.drawImage(surf.offscreen_canvas, 0, 0, w, h);
        // Readback check: find first black pixel in last column and last row
        var checkData = exportCtx.getImageData(0, 0, w, h).data;
        var lastColBlack = 0, lastRowBlack = 0;
        for (var _cy = 0; _cy < h; _cy++) {
            var _ci = (_cy * w + (w-1)) * 4;
            if (checkData[_ci]===0 && checkData[_ci+1]===0 && checkData[_ci+2]===0) lastColBlack++;
        }
        for (var _cx = 0; _cx < w; _cx++) {
            var _ci2 = ((h-1) * w + _cx) * 4;
            if (checkData[_ci2]===0 && checkData[_ci2+1]===0 && checkData[_ci2+2]===0) lastRowBlack++;
        }
        console.log("[save] w=",w,"h=",h,
            "pixel[0,0]=",checkData[0],checkData[1],checkData[2],
            "pixel[w-1,0]=",checkData[(w-1)*4],checkData[(w-1)*4+1],checkData[(w-1)*4+2],
            "lastCol black px=",lastColBlack,"/",h,
            "lastRow black px=",lastRowBlack,"/",w);
    ...
    // ─── Спільна логіка: Surface → байтовий рядок Skulpt ───────────────────────
    // format: 'RGBA' (default), 'RGB', 'ARGB', 'BGRA', 'BGR', 'P' (=RGB)
    // flipped: якщо True — рядки перевернуті по вертикалі (OpenGL-порядок)
    function _surfToBytes(surf, format, flipped) {
        var fmt = (format !== undefined && format !== Sk.builtin.none.none$)
            ? Sk.ffi.remapToJs(format).toUpperCase() : 'RGBA';
        var flip = (flipped !== undefined && flipped !== Sk.builtin.none.none$)
            ? !!Sk.ffi.remapToJs(flipped) : false;
        ...
        var raw = surf.context2d.getImageData(0, 0, w, h).data; // Uint8ClampedArray, RGBA
        ...
        else if (fmt === 'RGBX') ch = [0,1,2,3]; // 4 байти, але альфа=255
        else throw new Sk.builtin.ValueError('Unsupported image format: ' + fmt);
        ...
        for (var row = 0; row < h; row++) {
            ...
            for (var x = 0; x < w; x++) {
                var src = (y * w + x) * 4;
                for (var c = 0; c < bpp; c++) {
                    var val = raw[src + ch[c]];
                    if (fmt === 'RGBX' && c === 3) val = 255;
                    out[idx++] = val;
                }
            }
        }

        // Конвертуємо Uint8Array → Skulpt bytes
        // У Skulpt bytes.v — це Uint8Array або масив цілих 0-255, НЕ JS-рядок
        return new Sk.builtin.bytes(out);
    }

    // ─── Допоміжна: будь-який Skulpt об'єкт даних → JS Uint8Array ───────────────
    function _pyDataToUint8(data) {
        var v = data.v;

        // 1. bytes / bytearray: .v є Uint8Array або Array of integers
        if (v instanceof Uint8Array || v instanceof Uint8ClampedArray) {
            return new Uint8Array(v);
        }
        if (Array.isArray(v)) {
            var arr = new Uint8Array(v.length);
            for (var i = 0; i < v.length; i++) {
                arr[i] = (typeof v[i] === 'number') ? (v[i] & 0xff)
                        : (Sk.ffi.remapToJs(v[i]) & 0xff);
            }
            return arr;
        }

        // 2. str (Python 2-style bytestring або tostring() результат):
        //    .v є JS string, кожен символ має code 0-255
        if (typeof v === 'string') {
            var arr2 = new Uint8Array(v.length);
            for (var i = 0; i < v.length; i++) arr2[i] = v.charCodeAt(i) & 0xff;
            return arr2;
        }

        // 3. Якщо .v не визначено — спробуємо через Skulpt bytes API
        if (data.tp$name === 'bytes' && typeof data.valueOf === 'function') {
            var jsVal = data.valueOf();
            if (typeof jsVal === 'string') {
                var arr3 = new Uint8Array(jsVal.length);
                for (var i = 0; i < jsVal.length; i++) arr3[i] = jsVal.charCodeAt(i) & 0xff;
                return arr3;
            }
        }

        throw new Sk.builtin.TypeError(
            'Expected bytes, bytearray or str, got ' + Sk.abstr.typeName(data));
    }

    // ─── Спільна логіка: байтовий рядок → Surface ───────────────────────────────
    function _bytesToSurf(data, size, format, flipped) {
        var fmt = (format !== undefined && format !== Sk.builtin.none.none$)
            ? Sk.ffi.remapToJs(format).toUpperCase() : 'RGBA';
        var flip = (flipped !== undefined && flipped !== Sk.builtin.none.none$)
            ? !!Sk.ffi.remapToJs(flipped) : false;
        ...
        var w   = (typeof sz[0] === 'number') ? sz[0] : Sk.ffi.remapToJs(sz[0]);
        var h   = (typeof sz[1] === 'number') ? sz[1] : Sk.ffi.remapToJs(sz[1]);

        var raw = _pyDataToUint8(data);

        // Порядок каналів → позиції в RGBA
        // ch[i] = індекс у вхідному пікселі, який йде у канал RGBA[i]
        // Визначаємо bpp та маппінг у RGBA
        var bpp, toRGBA;
        if (fmt === 'RGBA') {
            bpp = 4; toRGBA = function(p){ return [p[0],p[1],p[2],p[3]]; };
        } else if (fmt === 'RGB' || fmt === 'P') {
            bpp = 3; toRGBA = function(p){ return [p[0],p[1],p[2],255]; };
        } else if (fmt === 'ARGB') {
            bpp = 4; toRGBA = function(p){ return [p[1],p[2],p[3],p[0]]; };
        } else if (fmt === 'BGRA') {
            bpp = 4; toRGBA = function(p){ return [p[2],p[1],p[0],p[3]]; };
        } else if (fmt === 'BGR') {
            bpp = 3; toRGBA = function(p){ return [p[2],p[1],p[0],255]; };
        } else if (fmt === 'RGBX') {
            bpp = 4; toRGBA = function(p){ return [p[0],p[1],p[2],255]; };
        } else {
            throw new Sk.builtin.ValueError('Unsupported image format: ' + fmt);
        }

        var expected = w * h * bpp;
        if (raw.length < expected)
            throw new Sk.builtin.ValueError(
                'Not enough data: expected ' + expected + ' bytes, got ' + raw.length);

        var surf = Sk.misceval.callsim(PygameLib.SurfaceType,
            new Sk.builtin.tuple([Sk.ffi.remapToPy(w), Sk.ffi.remapToPy(h)]));
        var imgData = surf.context2d.createImageData(Math.round(w), Math.round(h));
        var dst = imgData.data;

        for (var row = 0; row < h; row++) {
            var y = flip ? (h - 1 - row) : row;
            for (var x = 0; x < w; x++) {
                var srcIdx = (row * w + x) * bpp;
                var p = [];
                for (var c = 0; c < bpp; c++) p.push(raw[srcIdx + c]);
                var rgba = toRGBA(p);
                var dstIdx = (y * w + x) * 4;
                dst[dstIdx]   = rgba[0];
                dst[dstIdx+1] = rgba[1];
    ...
    // pygame.image.tostring(Surface, format, flipped=False) -> bytes
    mod.tostring = new Sk.builtin.func(function (surf, format, flipped) {
        return _surfToBytes(surf, format, flipped);
    });

    // pygame.image.fromstring(bytes, size, format, flipped=False) -> Surface
    mod.fromstring = new Sk.builtin.func(function (data, size, format, flipped) {
        return _bytesToSurf(data, size, format, flipped);
    });

    // pygame.image.tobytes — аліас tostring (pygame 2.x назва)
    mod.tobytes = new Sk.builtin.func(function (surf, format, flipped) {
        return _surfToBytes(surf, format, flipped);
    });

    // pygame.image.frombytes — аліас fromstring (pygame 2.x назва)
    mod.frombytes = new Sk.builtin.func(function (data, size, format, flipped) {
        return _bytesToSurf(data, size, format, flipped);
    });

    // pygame.image.frombuffer(bytes_obj, size, format) -> Surface
    // Інтерпретує дані безпосередньо (без копії) — у браузері завжди копія
    mod.frombuffer = new Sk.builtin.func(function (data, size, format) {
        // flipped=False для буферів (OpenGL передає перевернуто, але pygame.image — ні)
        return _bytesToSurf(data, size, format, undefined);
    });

    // pygame.image.tobuffer(Surface, format, flipped=False) -> bytes
    mod.tobuffer = new Sk.builtin.func(function (surf, format, flipped) {
        return _surfToBytes(surf, format, flipped);
    });
