...
$loc.reshape = new Sk.builtin.func(function (self) {
    ...
    // 1. Парсинг order ('C', 'F', 'A')
    if (args.length > 1) {
        var last = args[args.length - 1];
        if (last instanceof Sk.builtin.str) {
            var o = Sk.ffi.remapToJs(last).toUpperCase();
            if (['C', 'F', 'A'].indexOf(o) !== -1) {
                order = o;
                args.pop();
            }
        }
    ...
    } else if (args.length === 1 && (args[0] instanceof Sk.builtin.tuple || args[0] instanceof Sk.builtin.list)) {
        shape = args[0];
    } else {
    ...
    **/
    ...
    $loc.__getitem__ = new Sk.builtin.func(function (self, index) {
      ...
      if (Sk.builtin.checkInt(index)) {
        ...
        // index like [1,3] — supports partial indexing (fewer indices than dims)
        var _tlen = Sk.ffi.remapToJs(Sk.builtin.len(index));
        var keyJs = [];
        for (var _ki = 0; _ki < _tlen; _ki++) {
          keyJs.push(Sk.ffi.remapToJs(index.mp$subscript(new Sk.builtin.int_(_ki))));
        }
        var _ndim = PyArray_NDIM(self);
        if (_tlen > _ndim) {
          throw new Sk.builtin.IndexError('too many indices for array');
        }
        // Compute flat offset using only the provided indices
        var _off = 0;
